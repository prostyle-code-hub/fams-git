...
class Crud_model extends CI_Model {
    ...
    public function get_school_stuff($stuff_id = 0) {
        $this->db->where('status', 1);
        return $this->db->get('school_stuff');
    }

    public function get_school_rules() {
        $this->db->where('status', 1);
        return $this->db->get('school_rule');
    }

    public function check_rule_status($rule_id, $user_id) {

        $this->db->where('rule', $rule_id);
        $this->db->where('agent_id', $user_id);
        //$this->db->get('school_rule_test');
        $row_count = $this->db->get('school_rule_test')->num_rows();
    ...
    public function get_single_slide($rule_id) {
        //echo $rule_id; exit();
        $query = $this->db
                ->select('*')
                ->from('school_rule')
    ...
    public function save_rule_results() {
        $data['rule'] = $this->input->post('rule');
//        $q1 = $this->input->post('question_1');
//        $q2 = $this->input->post('question_2');
//        $q3 = $this->input->post('question_3');
//        $answer_list = $q1 . ',' . $q2 . ',' . $q3;
        //$data['answer'] = $answer_list;
        $data['agent_id'] = $this->session->userdata('user_id');

        if ($this->input->post('rule') != "") {
            $this->db->insert('school_rule_test', $data);
        }
    }

    public function check_rule() {
        $this->db->select('read_rule');
        $this->db->where('user_id', $this->session->userdata('user_id'));
        print_r($this->db->get('agents')->row());
    }

    public function save_rule_status() {
    ...
    public function get_slide($slide_id = 0) {
        ...
        return $this->db->get('school_rule')->row();
    }
    ...
    public function rule_answer($rule_id) {
        ...
        $this->db->where('rule', $rule_id);
        $this->db->where('agent_id', $user_id);
        return $this->db->get('school_rule_test')->row();
    }

    public function next_page() { // Rules
        $this->db->order_by("id", "desc");
        return $this->db->get('school_rule')->row();
    }
    ...
    function forgot_password() {
        ...
        //$random_code = rand();
        $verification_code = rand(111111, 9999999);
    ...
    function change_password_from_forgot_passord($verification_code = "") {
        ...
        $new_verification_code = rand(111111, 9999999);

        //$this->db->where('email', $email);
        $this->db->where('verification_code', $verification_code);
        $this->db->update('users', array('password' => sha1($this->input->post('confirm_password')), 'verification_code' => $new_verification_code));
        return true;
    }
    ...
    public function count_school_active_rules(){
        $this->db->where('status',1);
        return $this->db->count_all('school_rule'); 
    }
}
